...
`drawMapCanvas` seam in `client-app.js`. 64px-cell default (ticket #37 —
enlarged from 32; configurable via `mapRenderConfig`), current z-plane only,
Hi-DPI via `devicePixelRatio`, no
game-engine dependency; the server map stays renderer-agnostic JSON.

Implemented (ticket #32): the "sprite tiles" path is real — cells blit from
the committed starter sheet (`public/tilesets/oathstar-starter-32x32/`,
generated by `bin/generate_oathstar_tileset.py`), resolved by tile NAME per
cell kind in the pure `src/client/tileset.js`, with the #16 flat-color draw
as the permanent fallback (no sheet, no errors, never a blank map). Crisp
pixel-art scaling via `imageSmoothingEnabled=false` + `image-rendering:
pixelated`. See Decision 050 and `docs/map-system.md`.

Implemented (ticket #37): the map reads bigger — the on-page cell is 64px
(`DEFAULT_MAP_CONFIG.tilePixels`), a crisp integer 2× of the now-native 32px
source sheet (`oathstar-starter-32x32/`, generator `TILE=32` → 128×96). Client
+ asset only; the blank-colors contract (Decision 054) is unchanged, only
re-scaled. See Decision 055.
