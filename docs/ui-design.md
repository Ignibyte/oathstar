...
Implemented (sprite tiles): cells blit from an **author-provided tile sheet**
(`docs/tileset-contract.md`), resolved by tile NAME per cell kind in the pure
`src/client/tileset.js`, with the flat-color draw as the permanent fallback
(no sheet, no errors, never a blank map). Crisp pixel-art scaling via
`imageSmoothingEnabled=false` + `image-rendering: pixelated`. Real art is
author-provided (8px-native); the generated blank-colors sheet is retired. See
Decisions 050 + 059 and `docs/map-system.md`.

Implemented (ticket #37): the map reads bigger — the on-page cell is 64px
(`DEFAULT_MAP_CONFIG.tilePixels`), configurable via `mapRenderConfig`. This is
the render scale, decoupled from the source tile size (an 8px sprite upscales
nearest-neighbour into the cell). See Decisions 055 + 059.
