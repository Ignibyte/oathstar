//! The studio's section routes (ticket #49). `items` / `enemies` / `settings` are
//! Editor-gated "Coming soon" stubs (HTTP 200, never a 404) until their tickets
//! land. All are gated like the dashboard. (The Regions section moved to
//! `regions.rs` when it gained authoring — ticket #51.)
...
pub fn editor_gate(jar: &CookieJar, sessions: &SessionStore) -> Option<Principal> {
...
#[cfg(test)]
mod tests {
    use super::{enemies, items, settings};
    ...
    async fn an_owner_session_reaches_a_section() {
        ...
        let res = items(State(state), jar(Some(&cookie_header(&id))).await).await;
        assert_eq!(res.status(), StatusCode::OK);
    }
}
